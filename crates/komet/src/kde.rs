...
pub fn kde(
    ...
    let eval_min = x_min.unwrap_or(data_min - 3.0 * bw);
    let eval_max = x_max.unwrap_or(data_max + 3.0 * bw);
    
    let step = (eval_max - eval_min) / (num_points - 1) as f64;
    let x: Vec<f64> = (0..num_points)
        .map(|i| eval_min + step * i as f64)
        .collect();
