...
pub fn kde(input: &[u8]) -> Result<Vec<u8>, String> {
    ...
    match decoder.pull().unwrap() {
        Header::Array(Some(_len)) => {
            ...
            let bandwidth = if bandwidth_val < 0.0 {
                None
            } else {
                Some(bandwidth_val)
            };
            ...
            let x_min = if x_min_val == f64::NEG_INFINITY {
                None
            } else {
                Some(x_min_val)
            };
            ...
            let x_max = if x_max_val == f64::INFINITY {
                None
            } else {
                Some(x_max_val)
            };
            ...
            encoder
                .push(Header::Array(Some(kde_result.x.len())))
                .unwrap();
            ...
            encoder
                .push(Header::Array(Some(kde_result.y.len())))
                .unwrap();
            for y in kde_result.y {
                encoder.push(Header::Float(y)).unwrap();
            }

            encoder.text("bandwidth", None).unwrap();
            encoder.push(Header::Float(kde_result.bandwidth)).unwrap();
