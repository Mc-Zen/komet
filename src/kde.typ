...
///
/// Returns:
/// A dictionary with keys `x` and `y` containing arrays of x coordinates and corresponding
/// density values.
///
/// Example:
/// ```
...
#let kde(
  ...
  let bw = if bandwidth == auto { -1.0 } else { bandwidth }
  
  // Convert x-min: auto becomes negative infinity
  let xmin = if x-min == auto { calc.inf * -1 } else { x-min }
